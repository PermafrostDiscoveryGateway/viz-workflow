...
def start_actors_staging(staging_input_files_list):
    ...
    for _ in range(len(app_futures)):
        try:
            ...
            logging.info(f"work_queue.size() -- (if it's zero, we're just finishing existing tasks.): {str(work_queue.size())}")
            print(f"work_queue.size(): -- (if it's zero, we're just finishing existing tasks.) {str(work_queue.size())}")
