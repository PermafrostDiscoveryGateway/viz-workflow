...
def step0_staging():
    ...
    with open(
        os.path.join(config["dir_output"], "staging_input_files_list.json"), "w"
    ) as f:
...
def prep_only_high_ice_input_list():
    ...
    staging_input_files_list = prepend(
        staging_input_files_list_raw, CONFIG["dir_input"]
    )
...
def step2_raster_highest(batch_size=100):
    ...
    staged_path_json_filepath = os.path.join(
        config["dir_output"], "staged_paths_to_rasterize_highest.json"
    )
    print(
        ...
        try:
            ...
            for parent_tile_batch in parent_tile_batches:
                ...
                app_future = create_composite_geotiffs.remote(
                    parent_tile_batch, config, logging_dict=None
                )
                app_futures.append(app_future)
...
def step4_webtiles(batch_size_web_tiles=300):
    ...
    rasterizer.tiles.add_base_dir(
        "geotiff_remote_all_zs", config_new["dir_geotiff"], ".tif"
...
def start_logging():
    ...
    logging.basicConfig(
        level=logging.INFO,
        filename=CONFIG["dir_output"] + "workflow_log.txt",
        filemode="w",
