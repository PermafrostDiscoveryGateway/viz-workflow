...
def step0_staging():
    ...
    iwp_config["dir_footprints"] = iwp_config[
        "dir_footprints_local"
    ]  # reintroduce when processing IWP
    ...
    print(
        f"⚠️ Num missing footprints: {len(missing_footprints)}"
    )  # reintroduce when processing IWP
    ...
    with open(
        os.path.join(iwp_config["dir_output"], "staging_input_files_list.json"), "w"
    ) as f:
    ...
    try:
        ...
        for i in range(0, len(app_futures)):
            ...
            if any(err in ray.get(ready)[0][0] for err in ["FAILED", "Failed", "❌"]):
                ...
                print(
                    f"📌 Completed {i+1} of {len(staging_input_files_list)}, {(i+1)/len(staging_input_files_list)*100:.2f}%, ⏰ Elapsed time: {(time.time() - start)/60:.2f} min.\n🔮 Estimated total runtime: { ((time.time() - start)/60) / ((i+1)/len(staging_input_files_list)) :.2f} minutes.\n"
                )
        ...
        for failure in FAILURES:
            print(failure)  # for pretty-print newlines
...
def prep_only_high_ice_input_list():
    ...
    staging_input_files_list = prepend(
        staging_input_files_list_raw, IWP_CONFIG["dir_input"]
    )
    ...
    staging_input_files_list = list(
        filter(
            lambda filepath_str: any(
                keep_str in filepath_str
                for keep_str in ["high_ice/alaska", "water_clipped/alaska"]
            ),
...
def step2_raster_highest(batch_size=100):
    ...
    staged_path_json_filepath = os.path.join(
        iwp_config["dir_output"], "staged_paths_to_rasterize_highest.json"
    )
    print(
        f"Writing a copy of the files we're rasterizing to {staged_path_json_filepath}..."
    )
    ...
    print(
        f"The input to this step, Rasterization, is the output of Staging.\n Using Staging path: {iwp_config['dir_staged']}"
    )

    print(
        f"🌄 Rasterize total files {len(staged_paths)} gpkgs, using batch size: {batch_size}"
    )
    ...
    logging.info(
        f"total STAGED filepath batches (for raster_highest()): {len(staged_batches)} <-- total number of jobs to submit"
    )
    ...
    print(
        f"⏰ Total time to rasterize {len(staged_paths)} tiles: {(time.time() - start)/60:.2f} minutes\n"
    )
...
def step3_raster_lower(batch_size_geotiffs=20):
    ...
    """
    print(
        "3️⃣ Step 3: Create parent geotiffs for all lower z-levels (everything except highest zoom)"
    )
    ...
    iwp_config["dir_footprints"] = iwp_config[
        "dir_footprints_local"
    ]  # reintroduce when processing IWP data
    ...
    stager.tiles.add_base_dir(
        "geotiff_remote", iwp_config["dir_geotiff"], ".tif"
    )  # had to change this from geotiff to geotiff_remote bc got error that the geotiff base dir already existed
    ...
    for z in parent_zs:
        ...
        child_paths = stager.tiles.get_filenames_from_dir(
            base_dir="geotiff_remote", z=z + 1
        )
        ...
        print(
            f"📦 Rasterizing (Z-{z}) {len(child_paths)} children into {len(parent_tiles)} parents tifs (using {len(parent_tile_batches)} batches)"
        )
        ...
        try:
            ...
            for parent_tile_batch in parent_tile_batches:
                ...
                iwp_config["dir_footprints"] = iwp_config[
                    "dir_footprints_local"
                ]  # we deduplicate at rasterization, reintroduce when processing IWP data
            ...
            for i in range(0, len(app_futures)):
                ...
                print(f"✅ Finished {ray.get(ready)}")
                print(
                    f"📌 Completed {i+1} of {len(parent_tile_batches)}, {(i+1)/len(parent_tile_batches)*100:.2f}%"
                )
                ...
                print(
                    f"⏰ Running total of elapsed time: {(time.time() - start)/60:.2f} minutes. 🔮 Estimated time to completion: { ((time.time() - start)/60) / ((i+1)/len(parent_tile_batches)) :.2f} minutes.\n"
                )
                app_futures = not_ready
                if not app_futures:
                    break
    ...
    print(
        f"⏰ Total time to create parent geotiffs: {(time.time() - start)/60:.2f} minutes\n"
    )
    return "Done step 3."
...
def step4_webtiles(batch_size_web_tiles=300):
    ...
    print(
        f"Collecting all Geotiffs (.tif) in: {iwp_config['dir_geotiff']}..."
    ...
    rasterizer.tiles.add_base_dir(
        "geotiff_remote_all_zs", iwp_config_new["dir_geotiff"], ".tif"
    )  # call it something different than geotiff_remote because we already made that base dir earlier and it might not overwrite and might error cause already exists
    ...
    geotiff_paths = rasterizer.tiles.get_filenames_from_dir(
        base_dir="geotiff_remote_all_zs"
    )
    ...
    print(
        f"📦 Creating web tiles from geotiffs. Num parallel batches = {len(geotiff_batches)}"
    )
    ...
    print(
        f"⏰ Total time to create web tiles:  {len(geotiff_paths)} tiles: {(time.time() - start)/60:.2f} minutes\n"
    )
...
def stage_remote(filepath, config, logging_dict=None):
    ...
    return [filepath, socket.gethostbyname(socket.gethostname())]
...
def three_d_tile(input_file, filename, save_to):
    ...
    try:
        ...
        return [
            f"‼️ ‼️ ❌ ❌ ❌ ❌ -- THIS TASK FAILED ❌ ❌ ❌ ❌ ‼️ ‼️ with path: {save_to, filename} and \nError: {e}\nTraceback: {traceback.print_exc()}",
            socket.gethostbyname(socket.gethostname()),
        ]
...
def three_d_tile_batches(file_batch, output_dir):
    ...
    for filepath in file_batch:
        try:
            ...
            print(
                f"‼️ ‼️ ❌ ❌ ❌ ❌ -- THIS TASK FAILED ❌ ❌ ❌ ❌ ‼️ ‼️ with path (some part of this list): {filepath} and \nError: {e}\nTraceback: {traceback.print_exc()}",
                socket.gethostbyname(socket.gethostname()),
            )
...
def start_logging():
    ...
    logging.basicConfig(
        level=logging.INFO,
        filename=IWP_CONFIG["dir_output"] + "workflow_log.txt",
        filemode="w",
...
@ray.remote
def rsync_raster_to_scatch(
    rsync_python_file="utilities/rsync_merge_raster_to_scratch.py",
):
    while True:
